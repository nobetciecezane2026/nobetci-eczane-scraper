...
def scrape_city(city, url):
    ...
    for line in lines:
        # 1. İSİM TESPİTİ (ECZANESİ kelimesi geçen kısa satırlar)
        ...
        if current:
            # 2. TELEFON TESPİTİ (İçinde çokça rakam olan satırlar)
            digits = "".join(filter(str.isdigit, line))
            if len(digits) >= 10:
                current["phone"] = digits
                continue
            
            # 3. ADRES TESPİTİ (İsim bulundu ama telefon henüz bulunmadıysa aradakiler adrestir)
            if not current["phone"]:
                # Eğer satır çok kısaysa (ilçe ismi olabilir) veya adresin devamıysa ekle
                if current["address"]:
                    current["address"] += " " + line
                else:
                    current["address"] = line
                
                # Basit ilçe ayıklama: Adres satırında "/" varsa öncesini ilçe kabul et
                if "/" in line:
                    parts = line.split("/")
                    if len(parts) >= 2:
...
print(f"\n[✓] Veriler güncellendi: {len(all_data)} şehir hazır.")
